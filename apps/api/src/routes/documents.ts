 ...
 * nothing else.
 *
 * The upload route's body stays outside Fastify's schema option — it is raw
 * bytes, not JSON, so there is nothing for a Zod schema to describe. Every
 * other route here validates natively, the same as `trucks.ts`/`loads.ts`;
 * see that file's module note for why.
 *
 * ---------------------------------------------------------------------------
...
/**
 * `unattached` stays a plain string, checked against `'true'` below exactly
 * as it always was — `z.coerce.boolean()` would also coerce `?unattached=
 * false` to `true`, since any non-empty string is truthy to `Boolean()`.
 */
const ListDocumentsQuerySchema = PageQuerySchema.extend({
  loadId: z.string().uuid().optional(),
...
const AttachDocumentSchema = z.object({ loadId: z.string().uuid() });
...
export async function documentRoutes(app: FastifyInstance) {
  ...
  server.get(
    '/v1/documents',
    { schema: { tags: ['Documents'], summary: 'List documents', querystring: ListDocumentsQuerySchema } },
    async (request) => {
      const s = await requireScope(request);
  ...
  server.get(
    '/v1/documents/:id',
    { schema: { tags: ['Documents'], summary: 'Get a document', params: IdParamSchema } },
    async (request) => {
      const s = await requireScope(request);
  ...
  server.get(
    ...
    { schema: { tags: ['Documents'], summary: "A document's bytes", params: IdParamSchema } },
    async (request, reply) => {
      const s = await requireScope(request);
      const { id } = request.params;
   ...
   */
  server.post(
    '/v1/documents/:id/attach',
    {
      schema: {
        tags: ['Documents'],
        summary: 'Attach a document to a load',
    ...
    async (request) => {
      ...
      const { id } = request.params;
      const { loadId } = request.body;

      try {
        await attachToLoad(s, id, loadId);
   ...
   */
  server.post(
    '/v1/documents/:id/manual-fields',
    {
      schema: {
        tags: ['Documents'],
        summary: "Type in what a document says",
        params: IdParamSchema,
        body: ManualFieldsSchema,
      },
    },
    async (request) => {
      const s = await requireScope(request);
      ...
      for (const [field, raw] of Object.entries(request.body.fields)) {
        if (!known.has(field)) {
          throw new HttpError(
            400,
            'unknown_field',
