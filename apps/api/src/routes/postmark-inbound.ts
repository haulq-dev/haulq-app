...
export async function postmarkInboundRoutes(app: FastifyInstance) {
  ...
  // `hide: true` — Postmark dictates this payload shape, not HaulQ, so it has
  // no business in a doc meant for people integrating with HaulQ's own API.
  // Still validated through the same schema as before, just not advertised.
  server.post(
    '/v1/webhooks/postmark-inbound',
    { schema: { hide: true, body: PostmarkInboundSchema } },
    async (request, reply) => {
    ...
    const org = slug ? await getOrgBySlug(app.db, slug) : undefined;
      if (!org) {
        // 200, not 4xx: Postmark retries a non-2xx delivery, and mail sent to
        // an address with no matching org will never gain one by being
        // retried.
      ...
      const s = scope(app.db, {
        ...
        // A fresh id, same as every other route — this ties together the
        // events *this request* produces. Postmark's MessageID is not a UUID
        // and is not what correlationId is for; it is carried separately as
        // intakeMessageId, which is what traces a document back to the
        // mailbox.
      ...
      for (const attachment of payload.Attachments) {
        ...
        // The bytes decide, same as a direct upload — Postmark's ContentType
        // is a claim, and a mail client mislabels attachments as often as a
        // browser.
        ...
        try {
          ...
          if (deduped) {
            // Lost a race with a concurrent delivery of the same bytes —
            // Postmark redelivering the same message concurrently, most
            // likely.
          ...
          if (err instanceof DocumentError) {
            // A rule this attachment breaks, not infrastructure — retrying
            // the whole message would fail the same way again. Skip it and
            // keep going; the rest of the message's attachments may be fine.
            request.log.warn(
          ...
          // Unexpected — storage or the database is in trouble. Let this
          // throw to a 500 so Postmark retries the whole message; dedupe
          // makes that free for whatever already made it in.
          throw err;
        }
      }
